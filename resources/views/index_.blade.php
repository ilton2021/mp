	  ...
	  <input type="hidden" name="_token" value="{{ csrf_token() }}">
	      <center>
		   <table class="table table-bordered" style="width: 1000px;" cellspacing="0"> 
			...
			<tr>
			  @if($mp->tipo_mp == 0)
			  <td colspan="2"><center><strong><h3><br>Validar - Movimentação de Pessoal - ORDINÁRIO</h3></strong></center></td>	 
			  @else
			  <td colspan="2"><center><strong><h3><br>Validar - Movimentação de Pessoal - NÃO ORDINÁRIO</h3></strong></center></td>	  	  
			  @endif
		   ...
		   <table class="table table-bordered" style="width: 1000px;" cellspacing="0">
		    ...
		    <tr>
			 ...
			 @if(Auth::user()->funcao == "RH" || $mps[0]->solicitante == Auth::user()->name)
			  @if(empty($aprovacao))
			   <br><center><a class="btn btn-primary" href="{{ route('alterarMPAdmissao', array($mp->id, $adm->id)) }}">Alterar</a>	</center></td>
			  @else 
			 ...
			 <td colspan="1" width="1050">Cargo: <input class="form-control" type="text" id="cargo" name="cargo" value="<?php echo $adm->cargo; ?>" readonly="true" /></td>
			 <td width="370">
			 Remuneração Total:
			 ...
			 @endif
			 @if($s6 == true)
			  <input type='checkbox' id="g_6" name="g_6" disabled="true" checked title="NENHUMA DAS RESPOSTAS" /> N. D. R
			 @endif
			 @endif
			 </td>
		 ...
		 @if(!empty($demissao))		
		  ...
		  <table class="table table-bordered" style="width: 1000px;" cellspacing="0">
		   <tr>
			...
			@if(Auth::user()->funcao == "RH"  || $mps[0]->solicitante == Auth::user()->name)
			 @if(empty($aprovacao))
			  <br><center><a class="btn btn-primary" href="{{ route('alterarMPDemissao', array($mp->id, $dem->id)) }}">Alterar</a>	</center></td>
			 @else 
			  @foreach($aprovacao as $ap)
				<?php if($ap->resposta == 3){ $a = 1; } ?>
			  @endforeach	
		 ...
		 @if(!empty($alteracaoF))
		  ...
		  <table class="table table-bordered" style="width: 1000px;" cellspacing="0">
		   <tr>
			...
			@if(Auth::user()->funcao == "RH"  || $mps[0]->solicitante == Auth::user()->name)
			 @if(empty($aprovacao))
			  <br><center><a class="btn btn-primary" href="{{ route('alterarMPAlteracao', array($mp->id, $altF->id)) }}">Alterar</a></center>
			 @else 
			  @foreach($aprovacao as $ap)
				<?php if($ap->resposta == 3){ $a = 1; } ?>
			  @endforeach	
			 ...
			 @endif
			@endif
			<td width="600">Transferência proposta: Indique a Unidade 
		   ...
		   <tr>
			...
			<td colspan="1" width="450">Cargo Proposto: <input class="form-control" type="text" id="cargo_novo" name="cargo_novo" value="<?php echo $altF->cargo_novo; ?>" disabled="true" /></td>
			<td width="50">Novo Horário de Trabalho <input class="form-control" style="width: 200px;" type="text" id="horario_novo" name="horario_novo" value="<?php echo $altF->horario_novo; ?>" disabled="true" /></td>
		   </tr>
		   <tr> 
		 ...
		 @endif
		  ...
		  <table class="table table-bordered" style="width: 1000px;" cellspacing="0">
		   <tr>
			...
			<td><textarea type="text" id="descricao" name="descricao" class="form-control" required rows="10" cols="60" value="<?php echo $justificativa[0]->descricao; ?>" readonly="true">{{ $justificativa[0]->descricao }}</textarea></td>
		   </tr>
		  </table>
		  </center>
