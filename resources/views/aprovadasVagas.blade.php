					...
					<tr>
						...
						<tr>
							...
							@foreach($vagas as $vaga)
								<tbody>
								<tr>
								<td title="<?php echo $vaga->vaga; ?>"><center>{{ strtoupper(substr($vaga->vaga, 0, 17)) }}</center></td>
								<td><center>{{ $vaga->numeroVaga }}</center></td>
								<td title="<?php echo $vaga->solicitante; ?>"><center>{{ strtoupper(substr($vaga->solicitante, 0, 20)) }}</center></td> 
								<td><center><a href="{{ route('visualizarVaga', $vaga->id) }}" class="btn-info btn btn-sm">VISUALIZAR</center></a></td>
								</tr>
								</tbody>
							@endforeach
						</td>
