    ...
    <div class="row d-flex justify-content-center">
        <div class="col-xl-5 col-lg-6 col-md-7">
            <div class="card b-0">
                ...
                <fieldset class="show">
                  <div class="form-card">
				  <form action="{{ \Request::route('updatePDs') }}" method="post">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
						...
						<tr>
							...
							<td><font size="2"><b>Solicitante:</b></font><input readonly class="form-control form-control-sm" type="text" id="solicitante" name="solicitante" required value="<?php echo $pd[0]->solicitante; ?>" /></td>
							</tr>
							<tr>
							<td colspan="1"><font size="2"><b>Vaga:</b></font><input class="form-control form-control-sm" type="text" id="vaga" name="vaga" required value="<?php echo $pd[0]->vaga; ?>" readonly /></td>
							<td><font size="2"><b>Código da Vaga:</b></font><input class="form-control form-control-sm" type="text" id="codigo_vaga" name="codigo_vaga" value="<?php echo $pd[0]->codigo_vaga; ?>" readonly /> </td>
							...
							<tr>
							<td><font size="2"><b>Departamento:</b></font><input class="form-control form-control-sm" type="text" id="area" name="area" value="<?php echo $pd[0]->departamento; ?>" readonly /></td>
							<td><font size="2"><b>Data Emissão:</b></font><input class="form-control form-control-sm" type="text" id="data_emissao" name="data_emissao" readonly value="<?php echo date('d-m-Y', strtotime($pd[0]->data_emissao)); ?>" /></td>
							<td><font size="2"><b>Data Prevista:</b></font><input class="form-control form-control-sm" type="text" id="data_prevista" name="data_prevista" readonly required value="<?php echo date('d-m-Y', strtotime($pd[0]->data_prevista)); ?>" /></td>
							</tr>
						</table>
						</center> 
						...
						<table class="table table-bordered">
							...
							<tr>
							<td rowspan="5" width="150"><center><font size="3"><b>Abertura de Vaga</b></font></center>
							<?php $a = 0; ?> <br>
							<center>
							@if(Auth::user()->id != 198 && Auth::user()->id != 59 && Auth::user()->id != 60 && Auth::user()->id != 61 && Auth::user()->id != 155 && Auth::user()->id != 160 && Auth::user()->id != 5 && Auth::user()->id != 166 && Auth::user()->id != 183 && Auth::user()->id != 30)
								@if(empty($aprovacao))
									<a class="btn btn-primary btn-sm" href="{{ route('alterarPD', $pd[0]->id) }}">ALTERAR</a></td>
								@else 
							...
							<td><font size="2"><b>Salário:</b></font><input class="form-control form-control-sm" type="number" readonly id="salario" name="salario" value="<?php echo $pd[0]->salario; ?>" title="ex: 2500 ou 2580,21" /></td>
						...
						<tr>
							...
							<td><textarea type="text" id="descricao" name="descricao" class="form-control form-control-sm" required rows="4" cols="60" value="<?php echo $just_vaga[0]->descricao; ?>" readonly> {{ $just_vaga[0]->descricao }} </textarea></td>
						</tr>
						</table>
						</center>
						...
						<tr>
							...
							<input readonly type="text" id="justificativa" name="justificativa" class="form-control form-control-sm" value="<?php echo $ap->motivo; ?>" />
							@endif
							@endforeach
							</td>
							<td>
						...
						<tr>
							<td><font size="2">GESOR DE UNIDADE</font></td>
							<td>
							@foreach($aprovacao as $ap)
						...
						<table class="table table-bordered" style="height: 10px;">
						 <tr>
							<td align="right"> 
							 ...
							 @if(Auth::user()->id == $pd[0]->gestor_id || Auth::user()->id == 30) 
							   <a href="{{ route('n_autorizarPD', $pd[0]->id) }}" type="button" class="btn btn-danger btn-sm"> NÃO AUTORIZAR <i class="fas fa-times-circle"></i> </a>
							   <a href="{{ route('autorizarPD', $pd[0]->id) }}" type="button" class="btn btn-success btn-sm"> AUTORIZAR <i class="fas fa-check"></i> </a>
							 @endif
							</td>
						 </tr>
						</table>
