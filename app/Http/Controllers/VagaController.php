...
class VagaController extends Controller
{
	...
	public function minhasVagas()
	{
		try{
			...
			$validator = "Algo está errado!! Entre em contato com o suporte (Abra um helpdesk)!!";
			$unidades  = Unidade::all();
			$vagas 	   = Vaga::all();
	...
	public function pesquisaVagas(Request $request)
	{
		try{
			...
			$validator = "Algo está errado!! Entre em contato com o suporte (Abra um helpdesk)!!";
			$unidades  = Unidade::all();
			$vagas 	   = Vaga::where('id',0)->paginate(1);
			$aprovacao = Aprovacao::all();
			$gestores  = Gestor::all();
			$unidade_id = ""; $pesq = ""; $pesq2 = "";
			return view('criadasVagas', compact('unidades','vagas','aprovacao','gestores','unidade_id','pesq2','pesq'))
					->withErrors($validator);
		}
	}
	...
	public function autorizarVaga($id)
	{
		try {
			...
			$gestoresUnd = DB::select("SELECT * FROM gestor WHERE (unidade_id = ".$idU.") AND id <> 60 AND id <> 1 AND id <> 59 AND id <> 61 AND id <> 15 AND id <> 65 AND id <> 5 ORDER BY nome");
	...
	public function storeAutVaga($id, Request $request)
	{
		try {
			...
			if ($validator->fails()) {
				$aprovacao = AprovacaoVaga::where('id',0)->get();
				return view('home_autorizado_vaga', compact('unidade','vaga','gestores','gestoresUnd','aprovacao'))
					->withErrors($validator)
					->withInput(session()->flashInput($request->input()));	 
			} else {
				if(Auth::user()->funcao == "Superintendencia" || (Auth::user()->id == 61 && $vaga[0]->unidade_id != 8) || Auth::user()->id == 174) {
				$input['resposta'] = 3;
				DB::statement('UPDATE vaga SET concluida = 1 WHERE id = '.$id.';');
				DB::statement('UPDATE vaga SET aprovada = 1 WHERE id = '.$id.';');
				} else {
	...
	public function storeVaga($id_unidade, Request $request){
		try {
			$input 	   = $request->all();
			$input['inativa'] = 0;
			$unidade   = Unidade::where('id', $id_unidade)->get();
			...
			if ($validator->fails()) { 
				...
				if($input['escala_trabalho'] == "outra") {
					if(!empty($input['escala_trabalho2'])){
   ...
   public function duvidasVagas()
	...
	public function welcomeErroVaga()
	{
		return view('welcomeErroVaga');
	}
}